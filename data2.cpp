...
MyDate& MyDate::operator+(unsigned int days)
{
	if (!(year_ % 4))
	{
		unsigned int tmp2[12] = { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
		if (days > 366)
		{
			year_ += days / 366;
			days %= 366;
...
{
	 unsigned int tmp1[12] = { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
	day_++;
	if (!(year_ % 4))
	{
